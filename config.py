EMAIL_CONFIG_PATH = "email_config.json"
RESULT_PATH = "track.json"

LEVEL_ABSOLUTELY_UNIQUE = 1 # returns all results searched
LEVEL_UNIQUE = 2          # returns results with full keyword in their title
LEVEL_AMBIGUOUS = 3       # searches with supplemental keywords, returns results with full keyword in their title

SITE_MERCARI = "mercari"
SITE_YAHOO_AUCTIONS = "yahoo_auctions"

MERCARI_CATEGORY_CD = 75
YAHOO_CATEGORY_MUSIC = 22152
YAHOO_CATEGORY_CD = 22192

TRACK_STATUS_NEW = "New!!"
TRACK_STATUS_MODIFIED = "Modified"

REQUEST_INTERVAL = 1

ALL_ENTRIES = 0